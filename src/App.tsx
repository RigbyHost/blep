...
function App() {
  ...
  return (
      <SidebarProvider>
        <Sidebar variant="inset" className="relative z-20">
          <SidebarContent className="py-2">
            ...
            >
              ...
              <p className="text-xl font-semibold" data-tauri-drag-region>
                Серверы
              </p>
            </div>
            ...
            <div className="flex flex-col gap-2 p-2">
              {servers.map((server) => (
                  <Card
                      ...
                      className={[
                        "cursor-pointer py-2! gap-2 transition-colors",
                        "flex flex-col items-center justify-center text-center",
                        selectedServer?.server.srvid === server.server.srvid
                            ? "bg-white/10"
                            : "",
                      ].join(" ")}
        ...
        <SidebarInset className="relative overflow-hidden">
          {selectedServer && (
              <div
                  className="absolute inset-0 w-full h-full z-0 bg-cover bg-center transition-all duration-700 ease-in-out pointer-events-none"
                  style={{
          ...
          <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
            <DialogContent className="sm:max-w-[425px]">
              ...
              <div className="grid gap-4">
                ...
                {patching && (
                    <p className="text-sm text-gray-500 text-center">{patchStatus}</p>
                )}
              </div>
          ...
          <div className="flex flex-col w-full h-full items-center justify-center p-8 relative overflow-hidden pointer-events-none z-10">
            {selectedServer ? (
                <div className="flex flex-col items-center gap-6 relative z-10 pointer-events-auto">
                  <img
                      src={selectedServer.server.icon}
                      className="w-32 h-32 rounded-3xl shadow-2xl"
                      alt={selectedServer.server.srvName}
                  />
                  <div className="text-center space-y-2">
                    <h1 className="text-4xl font-bold tracking-tight text-white drop-shadow-md">
                      {selectedServer.server.srvName}
                    </h1>
                  ...
                  <Button
                      size="lg"
                      className="w-48 text-lg h-12 shadow-xl"
